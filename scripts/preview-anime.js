...
(async()=>{
 const profile=fs.mkdtempSync(path.join(os.tmpdir(),'elysium-anime-preview-'));
 const instance=await electron.launch({args:[path.resolve('.')],env:{...process.env,ELYSIUM_TEST_PROFILE:profile}});
 try{
  ...
  console.log('state',await page.evaluate(async()=>{const s=await window.elysium.getState();return {tabCount:s.tabs.length,workspaceCount:s.workspaces.length,ui:s.ui};}));
