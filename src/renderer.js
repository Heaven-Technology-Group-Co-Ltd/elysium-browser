...
const svg = name => `<svg viewBox="0 0 24 24" aria-hidden="true">${window.elysiumIcons[name] || window.elysiumIcons.globe}</svg>`;
...
const themeCatalog = window.elysiumThemes.catalog;
...
async function command(action,payload) {
  try { const r=await window.elysium.command(action,payload); if(!r.ok) toast(r.error); return r; }
  catch { toast('ไม่สามารถติดต่อแอปได้ กรุณาเปิด elysium-browser อีกครั้ง'); return {ok:false}; }
}
...
function render(next) {
  ...
  $('.brand-signature').setAttribute('aria-label',`elysium-browser ${visibleVersion}`);
  const page=tab.url.startsWith('elysium://')?tab.url.slice('elysium://'.length):'web';
  ...
  if(document.activeElement!==$('#address')||shownTabId!==tab.id)$('#address').value=tab.url==='elysium://home'?'':tab.url;
...
function renderHomeRefresh(){
 ...
 const recentTabs=state.tabs.filter(t=>!t.url.startsWith('elysium://')&&t.workspaceId===state.activeWorkspace).slice(-3).reverse();
 ...
 $('#content').innerHTML=`<div class="newtab">${active().private?'<div class="private-banner">Private session · ไม่เก็บประวัติหรือ restore แท็บนี้ · บุ๊กมาร์ก/ไฟล์ดาวน์โหลดที่สั่งเก็บยังคงอยู่ และเว็บไซต์/เครือข่ายยังเห็นการเชื่อมต่อได้</div>':''}
  <section class="hero"><div class="hero-scene"></div><img class="hero-character" src="${art('cherry-home-short')}" alt="elysium-browser ผู้ช่วยผมบ๊อบสั้นสีน้ำเงินกำลังทำงาน"><div class="hero-copy"><p class="eyebrow">GOOD MORNING, EXPLORER</p><h1>ค้นพบโลกที่กว้างกว่าเดิม<br>ไปกับ <span>elysium-browser</span></h1><p class="hero-subtitle">พื้นที่สำหรับการค้นหา ไอเดีย และสิ่งที่คุณสนใจ</p><form id="home-search" class="hero-search">${svg('search')}<div class="search-copy"><label for="search-input">ค้นหาด้วย <strong>elysium-browser</strong> หรือพิมพ์ URL</label><input id="search-input" aria-label="ค้นหาด้วย elysium-browser หรือพิมพ์ URL" placeholder="Search with elysium-browser or type a URL" autocomplete="off"></div><button class="search-submit" type="submit" aria-label="ค้นหาหรือเปิดเว็บไซต์">${svg('arrow')}</button></form><p class="search-provider">ค้นหาด้วย ${engines[state.settings.searchEngine]} · แยกคำค้นและ URL ด้วยระบบเดิม</p></div></section>
  ...
  <aside class="home-rail"><article class="card cyrvor-card"><img src="${art('cyrvor-brand')}" alt="CYRVOR — Autonomous Intelligence for Cyber Defense"><span>CYRVOR</span></article><section class="card assistant-card"><img src="${art('cherry-assistant-short')}" alt="elysium-browser ผู้ช่วยอนิเมะผมบ๊อบสั้นสีน้ำเงิน"><div><small>elysium-browser AI</small><strong>พร้อมช่วยจัดระเบียบ<br>ความคิดของคุณ</strong><p class="assistant-status">${aiReady?'เชื่อมต่อ provider แล้ว':'ยังไม่ได้เชื่อมต่อ AI'}</p><button class="secondary-button" data-panel="ai">${aiReady?'เปิด AI Assist':'ดูวิธีเชื่อมต่อ'} ${svg('arrow')}</button></div></section><section class="card workspace-rail"><div class="card-head"><h2>${svg('grid')}Recent workspaces</h2><button class="text-button" data-page="workspaces">ดูทั้งหมด ${svg('arrow')}</button></div><div class="workspace-rail-list">${state.workspaces.slice(0,3).map(w=>`<button class="workspace-rail-item ${w.id===state.activeWorkspace?'active':''}" data-workspace="${esc(w.id)}"><span class="workspace-mark" style="color:${esc(w.color)}">${svg('briefcase')}</span><span><strong>${esc(w.name)}</strong><small>${state.tabs.filter(t=>t.workspaceId===w.id).length} แท็บ</small></span>${svg('arrow')}</button>`).join('')}<button class="workspace-rail-item workspace-create" data-action="new-workspace">${svg('plus')}<span><strong>สร้าง workspace</strong><small>แยกชุดแท็บสำหรับเรื่องใหม่</small></span></button></div></section></aside></div>
 <footer class="newtab-footer"><span>ELYSIUM-BROWSER <i>·</i> PEOPLE <i>·</i> IDEAS <i>·</i> A BRIGHTER TOMORROW</span><span>ค้นหาด้วย ${engines[state.settings.searchEngine]} · ข้อมูลเก็บในเครื่อง</span></footer></div>`;
}
...
function renderHome(){
 ...
 <section class="card team-banner"><img src="${art('team-banner')}" alt="elysium-browser, Violet, Ghost และ Nova"><div><p>Different skills.<br><em>Same curiosity.</em></p><button class="team-button" data-page="themes">พบทีม elysium-browser ${svg('arrow')}</button></div></section>
...
function renderNotesPage(){
 ...
 if(organizerTab==='reminders')body=`<div class="organizer-actions"><button class="primary-button" data-action="new-reminder">${svg('plus')}เพิ่มแจ้งเตือน</button></div><p class="organizer-help">${state.notificationsSupported?'elysium-browser จะส่ง Windows notification เมื่อถึงเวลา แม้คุณกำลังเปิดเว็บไซต์อยู่':'ระบบยังเก็บวันเวลาไว้ แต่ Windows notification ไม่พร้อมบนเครื่องนี้'}</p>${reminderRows()}`;
 $('#content').innerHTML=`<section class="page notes-page">${pageHeader('KEEP A LITTLE INSPIRATION','Notes, Post-it & Reminder','เก็บไอเดียลงเครื่อง จดข้อความสั้นบนบอร์ด และตั้งเวลาให้ elysium-browser เตือน')}${stats}${organizerTabsMarkup()}${body}</section>`;
}
...
function renderPanel(){
 ...
 const tab=active();const names={ai:'elysium-browser AI',notes:'Notes · Post-it · เตือน',downloads:'Download Hub',reader:'Reader',info:'ข้อมูลเว็บไซต์'};$('#panel-title').textContent=names[panel];
...
async function perform(action){
 ...
 if(action==='reader-to-note'||action==='ai-to-note'){const content=action==='reader-to-note'?state.reader:{title:'elysium-browser AI',text:state.aiResult,url:state.aiPreview?.url};if(!content)return;const r=await command('note-save',{title:content.title,body:content.text,sourceURL:content.url});if(r.ok)toast('เก็บเป็นโน้ตแล้ว');return;}
 ...
 if(action==='ai-oauth-logout'){const r=await command('ai-oauth-logout');if(r.ok)toast('ออกจากบัญชี OAuth ใน elysium-browser แล้ว');return;}
...
document.addEventListener('click',async e=>{
 if(e.target.closest('#clear-history')){const period=$('#history-period').value;await confirmAction('ล้างประวัติการเข้าชม?','ลบรายการในช่วงเวลาที่เลือก บุ๊กมาร์กและข้อมูลเว็บไซต์ยังคงอยู่',()=>command('clear-history',period));}
  ...
  if(e.target.closest('#update-install'))await confirmAction('ติดตั้งอัปเดตเลย?','แอปจะปิดและเปิดใหม่เพื่อติดตั้งเวอร์ชันที่ดาวน์โหลดไว้',()=>command('update-install'));
  if(e.target.closest('#update-open-releases'))await command('update-open-releases');
});
let draggedTab;
