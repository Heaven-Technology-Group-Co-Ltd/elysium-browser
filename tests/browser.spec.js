...
async function capture(name,width=1440,height=900){
 await instance.evaluate(({BrowserWindow},{width,height})=>{const w=BrowserWindow.getAllWindows()[0];w.setContentSize(width,height);w.show();w.focus();},{width,height});
...
test.beforeAll(async()=>{
  ...
  if(req.url.startsWith('/download')){
   ...
   let sent=start;const timer=setInterval(()=>{const len=Math.min(65536,total-sent);if(len>0){res.write(Buffer.alloc(len,67));sent+=len;}if(sent>=total){clearInterval(timer);res.end();}},slow?250:20);res.on('close',()=>clearInterval(timer));return;
  }
...
test('downloads show actual progress and support runtime pause/resume',async()=>{
 await navigate(`${base}/`);const savePath=path.join(profile,'elysium-test.bin');await instance.evaluate(({session,webContents},{url,savePath})=>{session.fromPartition('persist:cherry-web').once('will-download',(_e,item)=>item.setSavePath(savePath));webContents.getAllWebContents().find(w=>w.getURL()===url).downloadURL(url+'download?slow');},{url:`${base}/`,savePath});
 ...
 await call('resume-download',d.id);await expect.poll(async()=>(await state()).downloads[0].state,{timeout:30000}).toBe('completed');expect(fs.statSync(savePath).size).toBe(2097152);
